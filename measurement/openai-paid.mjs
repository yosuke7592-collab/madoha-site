...
export class OpenAiPaidAdapter extends PaidAdapter {
  ...
  async execute(input, env) {
    ...
    let response;
    try { response = await this.fetchImpl(OPENAI_ENDPOINT, { method: 'POST', headers: { authorization: `Bearer ${env.OPENAI_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: OPENAI_MODEL, input: input.question_text, tools: [{ type: 'web_search' }], tool_choice: 'required', include: ['web_search_call.action.sources'], max_output_tokens: 600, store: false }) }); }
    ...
    const parts = (payload.output || []).flatMap(item => item.type === 'message' ? item.content || [] : []).filter(item => item.type === 'output_text');
    const raw_answer = parts.map(item => item.text || '').join('\n'); const sources = [];
    ...
    for (const call of (payload.output || []).filter(item => item.type === 'web_search_call')) for (const item of call.action?.sources || []) sources.push({ url: item.url, title: item.title, evidence: 'source' });
    ...
    return this.normalize(input, { raw_answer, sources, usage: { input_tokens: usage.input_tokens || 0, output_tokens: usage.output_tokens || 0, search_queries: searchQueries }, estimated_cost, raw_response_ref: payload.id || null, measured_at: this.now(), provider_metadata: { model: payload.model || OPENAI_MODEL, store: false } });
  }
}
